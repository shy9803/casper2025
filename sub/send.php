...
$select = strip_tags(htmlspecialchars($_POST['mb_type'])); //select문 값
	
// 이메일을 생성하고 메일을 전송하는 부분
$to = 'tuhe992jd@gmail.com'; // 받는 측의 이메일 주소를 기입하는 부분
$email_subject = "FROM:  $name"; // 메일 제목에 해당하는 부분
$email_select = "문의유형: $select"; //select 부분을 메일로 송달
$email_body = "본 메일은 홈페이지 폼메일로부터 전송된 이메일입니다..\n\n"."세부정보는 다음과 같습니다.\n\nSubject: $subject\n\nName: $name\n\nEmail: $email_address\n\nMessage:\n$message";
$headers = "Reply-To: $email_address\r"; // 답장 주소

mail($to,'=?UTF-8?B?'.base64_encode($email_subject).'?=',$email_select,$email_body,$headers);
return true;			
?>